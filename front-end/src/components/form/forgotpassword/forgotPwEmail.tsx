...
const ForgotPwEmail = () => {
  ...
  const onSubmit = async (data: any) => {
    ...
    setLoading(true);
    const res = await fetch(`${uri}/send-forgot-password-code`, {
      method: "PATCH",
      headers: {
    ...
    if (res.ok) {
      ...
      router.push("/api/forgotPassword/VerifynewPassword");
      setLoading(false);
    }
    if (!res.ok) {
  ...
  return (
    <div className="flex flex-col space-y-4 w-[50%] items-center">
      ...
      >
        ...
        />
        <button
          className=" hover:bg-slate-200 text-green-900 font-bold px-5 py-1 border border-slate-400 rounded-xl cursor-pointer"
          type="submit"
        >
